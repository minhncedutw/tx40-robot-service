...
bool TX60L::Login(const string& url, const string& userName, const string& password){
    ...
    mStopTimes = 0;
    GetRobotCartesianPosition(mPos);

	return mIsLoggedIn;
}

int TX60L::CountStopTimes() {
    if(!mIsLoggedIn)
        return false;

    // get current position
    std::vector<double> curPos;
    GetRobotCartesianPosition(curPos);

    // collate current position and recorded postion
    if (IsVectEqual(mPos, curPos)) {
        mStopTimes++;
    }
    else {
        mStopTimes = 0;
...
bool TX60L::ForwardKinematics(std::vector<double> jnts, std::vector<double> &pos){
    ...
    pos.push_back(R2D(Rx));
    pos.push_back(R2D(Ry));
    pos.push_back(R2D(Rz));
